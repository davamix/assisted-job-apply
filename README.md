...
- **TalentClue** is a public **Drupal 7** form (no login) reached from the job page's "Inscríbete" link;
  ...
  `country_sub` and `degree_sub` belong in `answers.json`. The **phone field rejects separators**
  (`+34 600 123 456` fails, `+34600123456` is fine), the CV upload is AJAX and is only done when its
  hidden `fid` stops being `0`, and
...
- **The screenshot caught a bug the EVENT log could not — and the error message then misdescribed it.** Every field reported filled and the phone still failed: TalentClue rejected the canonical `+34 600 123 456` inline, which would have blocked the submit. `inputValue()` said the field held exactly what was typed, because the complaint lived in a sibling error node — **reading the filled-form screenshot is the check that found it**. The message («debe ser numérico. Por ejemplo "0034678901234"») reads as *digits only*, so the first fix converted `+` → `00`. That was wider than necessary: the `+` is fine and the **separators** are what fail, as the first real submit showed when the applicant switched it back by hand. The adapter now strips separators and keeps the `+`. **An inline validation message is a hint about the rule, not the rule.**
